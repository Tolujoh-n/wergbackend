...
/**
 * Backfill: finalize orderbook vault for an already-resolved market (releases reserves + on-chain sweep).
 * Body: { chainMarketId } or { matchId } or { pollId }
 */
router.post('/orderbook/finalize-market', async (req, res) => {
  try {
    const { chainMarketId, matchId, pollId } = req.body || {};
    let item = null;
    if (matchId) {
      item = await Match.findById(matchId);
      if (!item) return res.status(404).json({ message: 'Match not found' });
      if (!item.isResolved) return res.status(400).json({ message: 'Match is not resolved' });
    } else if (pollId) {
    ...
    const result = await finalizeOrderbookMarketOnResolve(item);
    res.json(result);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
