...
async function signTradingVaultWithdrawPayload({
  userAddress,
  amountWei,
  nonce,
  deadlineSec,
  ...
  const wallet = getClaimSignerWallet();
  const messageHash = ethers.solidityPackedKeccak256(
    ['address', 'uint256', 'bytes32', 'uint256', 'uint256', 'address', 'uint256'],
    [
      userAddress,
      BigInt(amountWei),
      nonce,
...
async function signOrderbookPositionClaimPayload({
  userAddress,
  marketId,
  amountWei,
  positionKey,
  ...
  const wallet = getClaimSignerWallet();
  const messageHash = ethers.solidityPackedKeccak256(
    ['address', 'uint256', 'uint256', 'string', 'bytes32', 'uint256', 'uint256', 'address', 'uint256'],
    [
      userAddress,
      BigInt(marketId),
      BigInt(amountWei),
...
module.exports = {
  ...
  signTradingVaultWithdrawPayload,
